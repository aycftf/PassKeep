...
while true; do
	...
	if [ "$regen" == "n" ]; then
		echo -e "\n"
		#find exact file excluding other pass named scripts to delete automatically
		#Regex (Kind of) in bash
		#findfile=$(find . -type f -name "*pass*.gpg")
		#echo "$findfile"
		#ls -la
		#read -p "enter file name here: " namee
...
if [ "$reuse" == "y" ]; then
	...
	if [[ "$rename" == "y" || "$rename" == "Y" ]]; then
		...
		read -p "Enter Name Here (Ignore file trailer.. e.g. .txt/.gpg): " newName
		#In future maybe just .gpg but make sure no complication with decrypt script
		sudo scp "$current_dir""/NEWPASSWORD/pass${RANDOMINT}.txt.gpg" "$newName".txt.gpg
	...
	sudo gpg --decrypt "$newName".txt.gpg

	if ! [ $? == 1 ]; then
		sleep 1
		echo -e "${BOLD}${BLUE}  \n Decryption Works Sucessfully! Grats " && sleep 1 
...
if [ "$correct" == "y" ]; then
	...
	if [ "$rename" == "y" || "$rename" == "Y" ]; then
		...
		read -p "Enter Name Here (Ignore file trailer.. e.g. .txt/.gpg): " newName
		#In future maybe just .gpg but make sure no complication with decrypt script
		sudo scp "$current_dir""/NEWPASSWORD/pass${RANDOMINT}.txt" "$newName".txt.gpg
